...
FriendGate supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to FriendGate. An explicit empty list
...
FriendGate `server.trusted_proxies` with the Caddy address/private subnet so the
