...
export function createSendMessageTools(params: {
  ...
  const toolSendMessage: Tool = {
    ...
    description: [
      "Send a Markdown message through the Infinite AI Message Bus to the parent agent, all agents, or one stable delegated-agent id.",
      ...
      content: [
        {
          type: "text",
          text: [
            `Message sent to ${displayRecipientLabel(record.recipientId)} via Infinite AI Message Bus.`,
            `seq=${record.seq}`,
            `channel=${record.channel}`,
          ].join("\n"),
        },
