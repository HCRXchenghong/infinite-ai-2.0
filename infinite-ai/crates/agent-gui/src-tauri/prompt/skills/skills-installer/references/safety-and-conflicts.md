...
Use this reference before installing or replacing Infinite AI skills.
...
- Parse `name` and `description` before installing. For `README.md` without frontmatter, Infinite AI derives the skill name from the directory and loads the full README when enabled.
...
- Do not manually edit Infinite AI Settings. SkillsManager automatically enables newly installed skills for the current conversation when installation succeeds.

## After Install

Infinite AI discovers skills through a scan of the fixed skills root. After an install, Infinite AI refreshes discovery; if the UI still has stale state:
