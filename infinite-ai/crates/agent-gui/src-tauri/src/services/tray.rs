...
pub struct TrayMenuModel {
    ...
    /// 状态行后缀（如「远程已连接」）；None 时只显示 `Infinite AI <version>`。
...
pub fn apply_tray_menu(
    ...
    // 托盘图标附属状态。
    let tooltip = model.tooltip.as_deref().unwrap_or("Infinite AI");
...
fn compose_status_line(app_version: &str, status_suffix: Option<&str>) -> String {
    let base = format!("Infinite AI {app_version}");
    match status_suffix {
