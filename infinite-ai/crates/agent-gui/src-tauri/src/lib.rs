...
fn toggle_main_window(app: &tauri::AppHandle) {
    if let Some(window) = app.get_webview_window(MAIN_WINDOW_LABEL) {
        ...
        if visible && focused {
            ...
            eprintln!("failed to show Infinite AI window from global shortcut: {error}");
        }
    }
}
...
fn toggle_main_window_pin(app: &tauri::AppHandle) {
    if let Some(window) = app.get_webview_window(MAIN_WINDOW_LABEL) {
        ...
        match window.set_always_on_top(next) {
            Ok(()) => {
                ...
                if next {
                    if let Err(error) = show_main_window(app) {
                        eprintln!("failed to show Infinite AI window when pinning: {error}");
                    }
                }
            ...
            Err(error) => eprintln!("failed to toggle Infinite AI window pin: {error}"),
        }
    }
}
...
fn forward_app_action(
    ...
    if show_window {
        if let Err(error) = show_main_window(app) {
            eprintln!("failed to show Infinite AI window for action {action}: {error}");
        }
    }
...
fn dispatch_app_action(app: &tauri::AppHandle, action: AppAction) {
    match action {
        AppAction::Summon => {
            if let Err(error) = show_main_window(app) {
                eprintln!("failed to show Infinite AI window: {error}");
            }
        }
        ...
        AppAction::OpenDataDir => {
            ...
            match commands::settings::config_dir() {
                Ok(dir) => {
                    ...
                    {
                        eprintln!("failed to open Infinite AI data directory: {error}");
                    }
                }
                Err(error) => eprintln!("failed to resolve Infinite AI data directory: {error}"),
            }
        }
        AppAction::Quit => {
...
fn request_app_exit(
    ...
    if running_count > 0 {
        if let Err(error) = show_main_window(app) {
            eprintln!("failed to show Infinite AI window before terminal exit confirm: {error}");
        }
        ...
        .on_tray_icon_event(|tray, event| match event {
            TrayIconEvent::DoubleClick {
                ...
                if let Err(error) = show_main_window(tray.app_handle()) {
                    eprintln!("failed to show Infinite AI window from tray double-click: {error}");
                }
            }
            TrayIconEvent::Click {
                button: MouseButton::Left,
                ...
                if cfg!(target_os = "windows") {
                    if let Err(error) = show_main_window(tray.app_handle()) {
                        eprintln!("failed to show Infinite AI window from tray click: {error}");
                    }
                }
            }
            _ => {}
...
pub fn run() {
    // WebKitGTK bundles only the XIM module in the portable Linux package.
    // Route GTK text input through the running IBus XIM bridge so Chinese
    // composition works even when the host uses an AppImage runtime.
    #[cfg(target_os = "linux")]
    {
        std::env::set_var("GTK_IM_MODULE", "xim");
        // Keep Qt-based helper windows (for example native file dialogs) on
        // the same IBus session as the WebKitGTK main window.
            ...
            move |window, event| {
                ...
                if let WindowEvent::CloseRequested { api, .. } = event {
                    ...
                    if commands::app::is_close_window_exit(&close_window_behavior) {
                        ...
                        eprintln!("failed to hide Infinite AI window on close: {error}");
                    }
                }
            }
        })
    ...
    app.run(move |_app, event| match event {
        ...
        tauri::RunEvent::Reopen { .. } => {
            if let Err(error) = show_main_window(_app) {
                eprintln!("failed to show Infinite AI window from dock reopen: {error}");
            }
        }
        tauri::RunEvent::ExitRequested { api, .. } => {
            if !allow_exit.load(Ordering::SeqCst) {
                ...
                if running_count > 0 {
                    if let Err(error) = show_main_window(_app) {
                        eprintln!(
                            "failed to show Infinite AI window before terminal exit confirm: {error}"
                        );
                    }
