...
for command_name in curl find grep head realpath sed sha256sum; do
...
# AppImage's bundled WebKitGTK runtime does not always include the host's
# GTK IM modules.  Prefer XIM backed by the running IBus daemon so CJK
# composition works when users launch the AppImage directly (without a
# wrapper script).  Respect explicit values supplied by a user's desktop
# launcher or shell.
sed -i '/^set -e$/a\
