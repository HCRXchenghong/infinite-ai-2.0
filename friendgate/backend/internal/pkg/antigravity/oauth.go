...
const (
	...
	// Antigravity OAuth 客户端 ID 必须通过环境变量提供；凭证不写入仓库。
...
var (
	// ClientID 保留为可测试/兼容的运行时变量，生产环境从
	// ANTIGRAVITY_OAUTH_CLIENT_ID 读取。
...
func init() {
	...
	// 从环境变量读取 client_secret；仓库不内置 OAuth secret。
...
func getClientID() (string, error) {
	if value := strings.TrimSpace(os.Getenv(AntigravityOAuthClientIDEnv)); value != "" {
		return value, nil
	}
...
func BuildAuthorizationURL(state, codeChallenge string) string {
	params := url.Values{}
	params.Set("client_id", strings.TrimSpace(os.Getenv(AntigravityOAuthClientIDEnv)))
