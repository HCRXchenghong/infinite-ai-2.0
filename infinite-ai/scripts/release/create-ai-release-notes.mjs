...
function fallbackNotes() {
  ...
  return `# Infinite AI ${releaseVersion.releaseTag}\n\nRelease ${releaseVersion.releaseTag}.`;
}
...
function normalizeMarkdown(markdown) {
  ...
  if (!output.startsWith("#")) {
    output = `# Infinite AI ${releaseVersion.releaseTag}\n\n${output}`;
  }
  return `${output.trim()}\n`;
}
...
function collectContext() {
  ...
  const repository = process.env.GITHUB_REPOSITORY?.trim() || "HCRXchenghong/infinite-ai-2.0";
...
function buildPrompt(context) {
  return [
    ...
    "- Start with exactly this H1: # Infinite AI " + context.releaseTag,
