...
const CODE_REVIEW_OWNERSHIP_MARKER_CONTENT: &str =
    "{\"schemaVersion\":1,\"owner\":\"Infinite AI\",\"skill\":\"liveagent-code-review\"}\n";
...
pub(crate) fn ensure_not_builtin_skill_management_target(
    ...
    if is_protected {
        return Err(format!(
            "SkillsManager action={action} cannot modify built-in Skill \"{name}\". Built-in Skills are managed by Infinite AI; create or update a separate user Skill instead."
        ));
    }
    Ok(())
}
